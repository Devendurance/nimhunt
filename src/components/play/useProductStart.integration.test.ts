...
describe('actual authenticated gate, game lifecycle, and Practice route', () => {
  ...
  it('claims a reserved Gem slot from the verified CTA and keeps cancellation retryable', async () => {
    ...
    expect(reservedText).toContain('Your treasure is reserved and waiting for payout.')
    expect(reservedText).not.toMatch(/NIM received|paid|sent|payout complete|TREASURE DELIVERED|replay/i)
