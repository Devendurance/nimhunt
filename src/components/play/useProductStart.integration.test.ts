...
describe('actual authenticated gate, game lifecycle, and Practice route', () => {
  ...
  it('claims a reserved Gem slot from the verified CTA and keeps cancellation retryable', async () => {
    ...
    expect(reservedText).toContain('Daily rewards are paid in the next payout batch.')
    expect(reservedText).not.toMatch(/NIM received|instantly|sent to your wallet|payout complete|TREASURE DELIVERED|replay/i)
