...
- Vault freeze is `DurableExpeditionRun.terminal`, not `status`/`rewardStatus`. After `VAULT_GAMEPLAY_VERIFIED` the run stays `STARTED`/`NONE`, but `terminal = { type: 'VERIFIED', result }` is set. `appendCheckpoint` rejects with `RUN_NOT_ACTIVE` when `run.terminal` is set. There is no separate `verifiedAt` / `verificationStage` / `gameplayVerified` run field; `verifiedAt` lives only on `terminal.result`. Covered by `verifies Vault gameplay without making reward eligibility final`. Do not treat a verified Vault run as ordinary active gameplay; keep it only for the future run-bound `NIMHUNT_VAULT_SEAL_V1` flow.
