...
- Product `/verify` is mission-specific. `objectiveReached` is shrine/vault geometry for every mission and must not terminal-verify Gem Runner or Chest Hunter. Client terminal flush: Gem `MISSION_COMPLETE` (gems >= 6 alive), Chest `MISSION_COMPLETE` (chests >= 4 alive), Vault `VAULT_REACHED` (objectiveReached alive), or death. `RUN_INCOMPLETE` is recoverable: restore gameplay, do not lose proof, do not show reservation copy.
...
- `SUPABASE_SERVICE_ROLE_KEY` / `SUPABASE_SECRET_KEY` are server-only aliases. The browser never talks to ledger/proof tables; never prefix these with `VITE_`.
...
- Durable proof start uses `server/ledger/sql/002_expedition_proof.sql` plus runtime `003_expedition_proof_runtime.sql`: published blueprint content is immutable, one active publication exists per day/mission, challenge expiry is capped by UTC midnight, and start/checkpoint/verify/Vault RPCs lock the run then persist immutably.
- Both daily-ledger and expedition-proof SECURITY DEFINER SQL functions use `pg_catalog, public` search paths, schema-qualified protected references, and service-role-only execution grants. Proof tables use FORCE ROW LEVEL SECURITY with no anon/authenticated write policies.
...
- Proof HTTP is backend-agnostic. `MemoryProofService` stays sync for fast tests. `ProofService` is the async Postgres adapter. Product HTTP awaits either. Shared domain code (`checkpoint.ts`, `verify.ts`, `vaultSeal.ts`, `runProof.ts`) is not duplicated in SQL.
- `NIMHUNT_PROOF_BACKEND=memory` is development/test only. `NIMHUNT_PROOF_BACKEND=postgres` is allowed in preview/production. Missing postgres credentials return `PROOF_UNAVAILABLE` and must never fall back to memory.
- The Vite proof plugin must pass `loadEnv(..., '')` Supabase keys into `createDefaultProofService`. `readServerSupabaseConfig(process.env)` is not enough: Vite does not put non-`VITE_` `.env` keys on `process.env`. Postgres with missing credentials must stay `PROOF_UNAVAILABLE`, never memory.
- Live FORCE RLS is testable without `SUPABASE_ANON_KEY` by creating a disposable Auth user, using the service secret as `apikey` and the user JWT as `Authorization`. Authenticated reads return `[]`; inserts are `42501`; proof RPCs are `permission denied for function`.
- Postgres checkpoint append serializes on `expedition_runs` row lock. Exact retry is checked by `(run_id, previous_checkpoint_hash)` before stale-head rejection. Two different batches from the same previous hash: one advances, the other is `CHECKPOINT_MISMATCH`.
- node-pg must JSON.stringify arrays/objects and cast `::jsonb`. A JS array bound to a jsonb parameter is otherwise sent as a Postgres array and the RPC fails closed.
...
- The memory proof adapter is allowed only with explicit `NIMHUNT_PROOF_BACKEND=memory` in development/test. Preview/production must fail closed for memory. Postgres is the durable backend.
...
- Product checkpoints accept only `{ runId, previousCheckpointHash, MOVE actions }`. Identity comes from the HttpOnly run session. Exact retry of an already-committed batch returns the same acknowledgement before stale-hash rejection; a different batch from an old checkpoint is `CHECKPOINT_MISMATCH`.
- The product client records accepted moves at the scene accept point, batches 8, allows 16 unacked, and flushes on death / local mission complete / vault reached / leave. Transient network failures retry the exact in-flight batch. Unrecoverable proof failure is `PROOF_LOST` and does not convert the run to Practice or mark gameplay FAILED unless HP is 0.
- The Vite proof plugin owned-path allowlist must include every path `dispatchExpeditionHttp` serves. Omitting `/api/expeditions/checkpoint` made the first 8-move batch fall through to Vite; the client parsed a non-JSON 404/HTML body as `MALFORMED_RESPONSE` and entered `PROOF_LOST`. Session/origin/hash checks never ran because the handler never received the request.
- Practice (`?practice=`) and `?dev=game` must not send checkpoint or verify traffic. Mid-run reload still fails closed; unacked local actions are not trusted.
- Checkpoint DEV `hashMatch=no` was not a chain bug. After dispatch it compared client `previousCheckpointHash` (pre-append) to `run.checkpointHash` (post-append). Logs now use `clientPrev` / `serverAfterDispatch` / `prevEqualsCurrent`.
- Final verify accepts only `{ runId, checkpointHash }`. Identity comes from the HttpOnly run session. The server reconstructs the transcript from immutable batches, replays from the bound blueprint, and persists server-derived summary fields only.
- Gem Runner success is `gemsCollected >= 6 && hp > 0` → `COMPLETED` / `ELIGIBLE` / `VERIFIED_ELIGIBLE`. Chest Hunter is `chestsOpened >= 4 && hp > 0` with the same terminal. Vault gameplay is `objectiveReached && hp > 0` → `VAULT_GAMEPLAY_VERIFIED` with `rewardStatus = NONE`; do not reuse `NIMHUNT_VAULT_SEAL_PREVIEW`.
- Vault freeze is `DurableExpeditionRun.terminal`, not `status`/`rewardStatus`. After `VAULT_GAMEPLAY_VERIFIED` the run stays `STARTED`/`NONE`, but `terminal = { type: 'VERIFIED', result }` is set. `appendCheckpoint` rejects with `RUN_NOT_ACTIVE` when `run.terminal` is set. There is no separate `verifiedAt` / `verificationStage` / `gameplayVerified` run field; `verifiedAt` lives only on `terminal.result`. The run remains available only for `NIMHUNT_VAULT_SEAL_V1` continuation; `/active` still rejects it.
- Alive incomplete verify is `RUN_INCOMPLETE` and must not become `FAILED` or success. `hp == 0` is `FAILED`. Proof-aware leave is `ABANDONED`. Exact verify retry returns the stored result. Verification consumes no extra attempt.
...
- `VERIFIED_ELIGIBLE` / `VAULT_GAMEPLAY_VERIFIED` is a terminal client session owned by the current product run, not reconstructed through `/active`. After verify, do not re-resolve the product gate for that mounted run. A same-tab remount of the gate may keep the trusted `/verify` result visible without remounting Phaser. Fresh reload or a later visit to the completed `?run=&runId=` URL must still fail closed because `/active` rejects non-STARTED / already gameplay-started runs. Clear the remembered terminal only on deliberate Back to missions / Return to Hunt, not because search params briefly unmatch. `/active` semantics stay strict.
- Product Vault seal is `NIMHUNT_VAULT_SEAL_V1`, not the preview type. One strict parser/serializer binds wallet, runId, vault-breaker, Angkor Room 01 TEMPLE_VAULT, runChallenge, rules/room/blueprint versions, blueprintId/hash, and vaultCheckpointHash. Server supplies every field. Client may display/sign the exact canonical string only.
- `POST /api/expeditions/vault-seal/prepare` requires authenticated session + `VAULT_GAMEPLAY_VERIFIED`, is idempotent, does not auto-sign, and consumes no attempt. `POST /api/expeditions/vault-seal/verify` accepts only `{ payload, publicKey, signature }`, re-parses/re-serializes, and reuses `verifyNimiqSignedCanonicalMessage`. Derived address must equal payload wallet and durable run wallet. One durable `run.vaultSeal` proof per run; exact retry returns the stored result; invalid signatures stay retryable. Run A cannot satisfy Run B. Preview verifier still rejects V1 as `UNSUPPORTED_MESSAGE_TYPE`.
- Product Vault UX: TEMPLE VAULT REACHED / Vault gameplay verified / Seal treasure. While signing: Sealing treasure…. On verified: TREASURE SEALED, shortened wallet/hash, Verified ✓, and `Reward claim is not enabled in this build yet.` Cancellation keeps gameplay verified and the seal unverified. Practice/dev do not call product prepare/verify. Do not remount Phaser after Vault gameplay verification or seal.

## Live-validated Postgres proof

- Live Supabase `001`/`002`/`003` schema, FORCE RLS, and service-only SECURITY DEFINER RPCs are verified.
- Real-device postgres-backed Gem Runner, Chest Hunter, and Vault Breaker + `NIMHUNT_VAULT_SEAL_V1` are verified.
- Daily attempt accounting is 3/day, consumed at signed Start. Failed and abandoned runs still consume an attempt.
- `daily_reward_pools` is 69 slots/day. `daily_wallet_state.rewards_reserved` is at most 1 per wallet/day. No payout exists yet.
- Dev attempt reset (`NIMHUNT_ENABLE_DEV_RESET`) may zero `expeditions_started` only. It must not touch `rewards_reserved`, proof history, or bypass one-reward-per-wallet/day.
- Claims, 69-slot reservation, NIM transfer, treasury, and payout remain the next product slices.
